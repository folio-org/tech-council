# ui-gobi-settings module acceptance criteria template
...
* [x] Modules must declare all consumed interfaces in the Module Descriptor “requires” and “optional” sections
  Aside, I was surprised to discover that multiple different interfaces are responsible for endpoints that begin with `/gobi`, e.g. the endpoints `/gobi/validate` and `/gobi/orders` are provided by the interface `gobi` while `/gobi/orders/custom-mappings` is provided by `custom-mapping`.
