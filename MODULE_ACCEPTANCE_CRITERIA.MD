...
* [ ] Inclusion of third party dependencies complies with [ASF 3rd Party License Policy](https://apache.org/legal/resolved.html), as defined and reported by the [FOLIO Module Evaluator](https://github.com/folio-org/tc-module-eval) tool (2)
  * GitHub Actions automated evaluation result page (linked in the Jira template) shows a passing evaluation.
  * _note: See license-categories.json (LINK TBD) for licenses which are allowed (Category A), prohibited (Category X), and allowed when [appropriately labelled in the module's README](https://apache.org/legal/resolved.html#appropriately-labelled-condition) (Category B-WCL).  See special-exceptions.json (LINK TBD) for permitted exceptions._
